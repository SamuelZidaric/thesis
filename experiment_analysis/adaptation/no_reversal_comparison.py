...
def gather_no_reversal_instances(file_paths, group_label):
    ...
    for file_path in file_paths:
        ...
        already_reversing = df[df['Status/Reaction Time'] == 'No Reversal']
...
group_1_no_reversal = gather_no_reversal_instances(rev_reaction_paths_list1, 'ATR+')
group_2_no_reversal = gather_no_reversal_instances(rev_reaction_paths_list2, 'ATR-')

# Combine instances from both groups
all_reversing_instances = pd.concat([group_1_no_reversal, group_2_no_reversal], ignore_index=True)

plt.figure(figsize=(12, 8))

# Create the countplot
countplot = sns.countplot(x='Group', data=all_reversing_instances, palette='Set1')
